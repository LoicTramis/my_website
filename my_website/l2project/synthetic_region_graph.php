<?php
	...
	if (isset($_GET["sort"]) && $_GET["sort"] == "TRUE") {
		$eta_per_region = display_all_regions($_GET["sort"]);
	} else {
		$eta_per_region = display_all_regions(FALSE);
	}
	$datax= array();
	$datay= array();
	
	foreach ($eta_per_region as $key => $value) {
		array_push($datay, $value);
		array_push($datax, $key);
	}
	
	// Size of graph
	$width = max($eta_per_region);
	$height = (count($eta_per_region) * 40);
...
// 	$graph->legend->SetFont(FF_ARIAL,FS_NORMAL, 10);
	...
	// Setup the titles
	$graph->title->Set("Nombre d'établissement par région");
	// Display the graph
	$graph->Stroke();		
?>